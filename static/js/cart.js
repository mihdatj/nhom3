...
function createCartItemHTML(item, index) {
    // QUAN TRỌNG: Sử dụng dữ liệu đã lưu trong giỏ hàng, KHÔNG tìm lại từ mảng products
    // Đảm bảo ID là số
    const itemId = Number(item.id);
    
    // Ensure all required properties exist - sử dụng dữ liệu đã lưu
    const itemName = item.name || 'Sản phẩm không tên';
    const itemPrice = item.price || 0;
    const itemImage = item.image || 'https://via.placeholder.com/80x80/CCCCCC/666666?text=No+Image';
    const itemVariant = item.variant || 'Mặc định';
    ...
    console.log(`🎨 Creating cart item HTML for: ${itemName}`);
    console.log(`   Item ID: ${itemId} (from item.id: ${item.id})`);
    ...
    // Use actualQuantity for calculations and display - sử dụng dữ liệu đã lưu
    const total = itemPrice * actualQuantity;
    const stockStatus = itemStock > 0 ? 'in-stock' : 'out-of-stock';
    const stockText = itemStock > 0 ? `Còn ${itemStock} sản phẩm` : 'Hết hàng';
    
    return `
        <div class="cart-item" data-index="${index}" data-product-id="${itemId}">
            <div class="cart-item-left">
                <div class="cart-item-checkbox">
                    <input type="checkbox" class="item-checkbox" data-index="${index}" onchange="updateCartSummary()" checked>
                </div>
                <img src="${itemImage}" alt="${itemName}" class="cart-item-image" onerror="this.src='https://via.placeholder.com/80x80/CCCCCC/666666?text=No+Image'">
                <div class="cart-item-info">
                    <div class="cart-item-name">${itemName}</div>
                    <div class="cart-item-variant">Phân Loại Hàng: ${itemVariant}</div>
            ...
            <div class="cart-item-quantity">
                <div class="quantity-controls">
                    ...
                    <input type="number" class="quantity-input" value="${actualQuantity}" min="1" max="${itemStock || 999}" onchange="setQuantity(${index}, this.value)">
                    <button class="quantity-btn" onclick="updateQuantity(${index}, 1)" ${actualQuantity >= (itemStock || 999) ? 'disabled' : ''}>+</button>
                </div>
...
function renderCartItems() {
    ...
    if (!container) {
        ...
        console.error('   Looking for element with ID: cartItemsContainer');
        return;
    }
    ...
    // Also read directly to compare
    const directRead = localStorage.getItem('cart');
    console.log('🔍 Direct localStorage read:', directRead ? directRead.substring(0, 200) : 'null');
    ...
    if (cartData && cartData.length > 0) {
        ...
        cartData.forEach((item, idx) => {
            console.log(`   [${idx}] ${item.name || 'Unknown'} - ID: ${item.id}, Qty: ${item.quantity}, Price: ${item.price}`);
        });
    }
    ...
    if (!cartData || !Array.isArray(cartData) || cartData.length === 0) {
        ...
        container.innerHTML = `
            <div class="empty-cart">
                ...
                <a href="/" class="empty-cart-btn">Tiếp tục mua sắm</a>
            </div>
        `;
        updateCartSummary();
        return;
...
function findSimilar(index) {
    ...
    if (item) {
        ...
        window.location.href = `/category?cat=${encodeURIComponent(item.category || '')}`;
    }
}
...
function refreshCart() {
    ...
    console.log('🔍 Raw cart from localStorage:', rawCart ? rawCart.substring(0, 200) : 'null');
    ...
    if (cartData && cartData.length > 0) {
        console.log('📋 Cart items:', cartData.map(item => `${item.name || 'Unknown'} (ID: ${item.id}) x${item.quantity || 0}`));
    }
    ...
    setTimeout(function() {
        ...
        if (cartData && cartData.length > 0) {
            cartData.forEach((item, index) => {
                ...
                // QUAN TRỌNG: Đảm bảo ID là số để so sánh chính xác
                ...
                // QUAN TRỌNG: Không tìm lại sản phẩm từ mảng products
                // Sử dụng dữ liệu đã lưu trong giỏ hàng để tránh hiển thị sai sản phẩm
                // Chỉ tìm lại nếu thiếu thông tin cơ bản
                if (!item.image || item.image === '') {
                    // Try to find image from products array ONLY if image is missing
                    if (typeof products !== 'undefined' && products.length > 0) {
                        // So sánh ID chính xác bằng cách convert sang số
                        const product = products.find(p => Number(p.id) === Number(item.id));
                        if (product && product.image) {
                            item.image = product.image;
                            hasChanges = true;
                            console.log(`Found image for item ${index} (ID: ${item.id}):`, product.image);
                        }
                    }
                ...
                // QUAN TRỌNG: Đảm bảo tên và giá được giữ nguyên từ dữ liệu đã lưu
                // KHÔNG tìm lại từ mảng products để tránh hiển thị sai sản phẩm
                if (!item.name) {
                    // Chỉ tìm lại nếu thực sự không có tên
                    if (typeof products !== 'undefined' && products.length > 0) {
                        const product = products.find(p => Number(p.id) === Number(item.id));
                        if (product && product.name) {
                            item.name = product.name;
                            hasChanges = true;
                            console.log(`Found name for item ${index} (ID: ${item.id}):`, product.name);
                        } else {
                ...
                if (!item.price) {
                    // Chỉ tìm lại nếu thực sự không có giá
                    if (typeof products !== 'undefined' && products.length > 0) {
                        const product = products.find(p => Number(p.id) === Number(item.id));
                        if (product && product.price) {
                            item.price = product.price;
                            hasChanges = true;
                            console.log(`Found price for item ${index} (ID: ${item.id}):`, product.price);
                        } else {
                            item.price = 0;
                            hasChanges = true;
                        }
